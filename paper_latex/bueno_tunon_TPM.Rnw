...
<<eval=FALSE>>=
#The user defines whether or not she would like to include the axis with the number of observations per window.
#Then she defines at which points
...
data <- as.data.frame(data[order(abs_running),])
  if (nr_obs==T) {
    ...
    nr_obs_lab <- cbind(nr_obs_lab, data$abs_running[nr_obs_lab])} #matrix to store the number of observations
                                                                   #at the points defined by the user.

#Finally, it is easy to include this information in the plot as an additional axis. 
axis(3, at=c(nr_obs_lab[,2]), labels=c(nr_obs_lab[,1]), cex=.6, col="grey50", 
           lwd = 0.5, padj=1, line=1, cex.axis=.7, col.axis="grey50")
      mtext("Number of observations", side=3, col="grey50", cex=.7, adj=0)} #axis caption
